...
Commit, with comment along these lines:

: Update to version X.

Then push =acorn_mos_refreshed=. Release should follow.

In the separate =acorn_mos_disassembly= working copy (don't do this in
the submodule), bump =refreshMinorVersion= in =src/refresh.65=.

Commit, with comment along these lines:

: Update due to release of version X.

Also not a bad time to do a =make dist=. (Commit with comment: =make
dist.=)

Then push =acorn_mos_disassembly=.
...
*** Sigh... printer bug part 2...

+2 max for the TIME value.
...
https://github.com/tom-seddon/acorn_mos_disassembly/issues/71

** OSBYTE/OSWORD

This handling subroutine could be inlined now.

: 11378 .f198                                           LF198:
: 11379 .f198           ca              dex                             dex
: 11380 .f199           10 03           bpl $f19e                       bpl LF19E
: 11381 .f19b           20 16 f3        jsr $f316                       jsr withTerminalROM
: 11382 .f19e                                           LF19E:
: 11383 .f19e           a6 f0           ldx $f0                         ldx originalX
: 11384 .f1a0           6c fa 00        jmp ($00fa)                     jmp (SEIWKA)

Called by the OSWORD/OSBYTE handling:

: 10431 .ee72           20 98 f1        jsr $f198                       jsr LF198

X is set by bit 14 of the routine address - auto-select Terminal ROM
depending on routine. There's a couple of =selectTerminalROMAndXXX=
routines in the osbyte/osword table. Could they be used instead?

** =selectTerminalROM=, =selectROMA=, etc.

Could inline calls on hot-looking paths?

** initWithRoutineStack and friends

Worth ekeing a bit more out of this?

* zx02

https://github.com/tom-seddon/acorn_mos_disassembly/issues/63

** Memory fill data

| What       | Uncompressed | Compressed | Notes                 |
|------------+--------------+------------+-----------------------|
| 16384 x 00 |        16384 |          7 | fill sideways bank    |
| 32768 x 00 |        32768 |          8 |                       |
| 57344 x 00 |        57344 |          8 | fill RAM as per LFC5A |
| Init RAM   |        57344 |         48 | has RTI at 0xd00      |
...
#+begin_src elisp
  ((nil . ((compile-command . "cd ~/beeb/mos320 && make tom_laptop"))))
#+end_src

* Terminal->Utils rename

LKG SHA1s

- 921bf8f071e48732ef737d9de003c597bc3fdb95 :: build/320r/320r.bin
- 855cf6b01fe68b75d8d1afc0fbfafce8fb9a12fe :: build/320r/320r_ide.bin
- 2a570b8032a50e05f4dd7d5157643c195d8d6b1d :: build/350r/350r.bin
- 082ce3d48f73194581cb7b6c5fe3ca36bc11c14b :: build/350r/350r_ide.bin
- 7ced39422510fa955058b79f6e779220aae87212 :: build/510r/510r.bin

* MOS 5.xx joystick stuff

OSBYTE 80 addresses: ($2b6 = adcResultLSBs)

- $2b6+2-1+N :: LSB
- $2b6+3-1+N :: MSB

| Xin | INX | &1 | ASL |   LSB |   MSB |
|-----+-----+----+-----+-------+-------|
|   1 |   2 |  0 |   0 | 0x2b7 | 0x2b8 |
|   2 |   3 |  1 |   2 | 0x2b9 | 0x2ba |
|   3 |   4 |  0 |   0 | 0x2b7 | 0x2b8 |
...
* MOS 3.50 vs Z80 second processor

Discussion: https://www.stardot.org.uk/forums/viewtopic.php?t=32471

Mini bootstrap ROM idea: https://www.stardot.org.uk/forums/viewtopic.php?p=477687#p477687

Possible mini bootstrap ROM contents

| Address | Bytes          | Instructions                   |
|---------+----------------+--------------------------------|
| bf00    | a9 8e          | lda #142                       |
| bf02    | 4c 0e bf       | jmp $bf0e                      |
| bf05    | --             | --                             k|
| bf06    | 42             | 6502 code, language entry only |
| bf07    | 09             | copyright offset               |
| bf08    | 00             | version number                 |
| bf09    | 00 28 43 29 00 | copyright message              |
| bf0e    | ?? 14 bf       | ldx $bf14                      |
| bf11    | 4c f4 ff       | jmp $fff4                      |
| bf14    | --             | OSBYTE 8E argument             |

The OSBYTE 8E argument is last, to simplify transferring the ROM: copy
the $13 fixed bytes, then do =lda $f4:ora #$20:sta $fee5= to send the
final one too.

The ROM doesn't need a Tube relocation address. The transfer and
execution will be handled manually.
